...
quill.root.addEventListener('dblclick', (e) => {
  ...
  showRenameInput(link);
});

function showRenameInput(link) {
  const rect = link.getBoundingClientRect();
  const editorRect = quill.root.getBoundingClientRect();

  renameInput.style.top =
    `${rect.top - editorRect.top - 4}px`;
  renameInput.style.left =
    `${rect.left - editorRect.left}px`;
  renameInput.style.width =
    `${rect.width + 8}px`;

  renameInput.value = activeFileLink.text;
  ...
  xhr.onload = () => {
	if (xhr.status === 200) {
		try {
			...
			class FileLinkBlot extends Inline {
			  static blotName = 'file-link';
			  static tagName = 'A';
			  ...
			  format(name, value) {
				// Empêche l’écrasement accidentel du lien
				if (name === 'link') return;
				super.format(name, value);
			  }
			}

			Quill.register(FileLinkBlot);
			...
			// espace après pour sortir du lien
